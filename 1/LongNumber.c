...
int compare(struct LongNumber a, struct LongNumber b)  // функция сравнения двух больших чисел
{   
...
{
	struct LongNumber remainder; // остаток от деления
...
struct LongNumber shiftLeft(struct LongNumber a, unsigned int s) // сдвигает число a на s разрядов влево
{
